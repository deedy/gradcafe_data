...
csv_name = 'all.csv'
exister = pd.read_csv(csv_name, sep=',', header = None, names = ['rowid', 'uni_name', 'major', 'degree', 'season', 'decision', 'decision_method', 'decision_date', 'decision_timestamp', 'ugrad_gpa','gre_verbal','gre_quant','gre_writing', 'is_new_gre', 'gre_subject', 'status', 'post_date', 'post_timestamp', 'comments'])
exist = exister.fillna("")
exist = exist.values
exidict = {}
dupes_errors= []
for ex in exist:
  ...
  if type(val) == float:
    val = str(int(val))
  if not val in exidict:
    exidict[val] = []
  exidict[val].append(ex)

def does_exist(check):
  global exidict
  check = [c if c!= None else "" for c in check]
  if not check[7] in exidict:
    return False
  exists = exidict[check[7]]
  num = [float(c) if c!= "" else "" for c in check[8:14]]
  try:
    is_in = [e for e in exists if e[18] == check[17] and e[1] == check[0] and e[2] == check[1] and e[3] == check[2] and e[4] == check[3] and e[9] == num[0] and e[10] == num[1] and e[11] == num[2] and e[12] == num[3] and e[13] == num[4]]
  except Exception as exc:
    Tracer()()
  if len(is_in) > 1:
    dupes_errors.append(is_in)
  return len(is_in) >= 1


data = []
for year in range(1, 699):
  with open('data/{0}.html'.format(year), 'r') as f:
    ...
    for tab in tables:
      ...
      for row in rows[1:]:
        ...
        if len(pro) > 0 and not does_exist(pro):
          print (count, pro)
          data.append(pro)
          count += 1
    print(year)
...
df.to_csv('all_i.csv', header = None)
